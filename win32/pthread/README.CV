...
x-cvsweb-markup&cvsroot=pthreads-win32
  ...
  */
  ...
  pthread_cleanup_push (__ptw32_cond_wait_cleanup, (void *)
&cleanup_args);
...
Sleep( 1 ); // @AT
      ...
      *
      * Note:
      *      __ptw32_sem_timedwait is a cancellation point,
      *      hence providing the
      ...
      *      if we are canceled.
      */
      if (__ptw32_sem_timedwait (&(cv->sema), abstime) == -1)         {
...
Sleep( 1 ); //@AT
      ...
      result = (__ptw32_increase_semaphore( &cv->sema, cv->waiters )
                 ? 0
                : EINVAL);
...
pthread_cond_destroy (pthread_cond_t * cond)
{
  ...
  if (*cond != (pthread_cond_t)  __PTW32_OBJECT_AUTO_INIT)
    {(*cond
      cv = *cond;
  ...
  else
    {
       ...
       */
      EnterCriticalSection(&__ptw32_cond_test_init_lock);
      ...
      LeaveCriticalSection(&__ptw32_cond_test_init_lock);
    }
                      ...
                      const struct timespec *abstime)
{
  ...
  if ((result = pthread_mutex_unlock (mutex)) == 0)
    {((result
       ...
       */
      if (__ptw32_sem_timedwait (&(cv->semBlockQueue), abstime) != 0)
        {(__ptw32_sem_timedwait
          result =  __PTW32_GET_ERRNO();
        }
    }
...
static int
__ptw32_cond_unblock (pthread_cond_t * cond,
                    int unblockAll)
{
  int result;
  pthread_cond_t cv;
   ...
   */
  if (cv == (pthread_cond_t)  __PTW32_OBJECT_AUTO_INIT)
    {(cv
      return 0;
    }
...
pthread_cond_broadcast (pthread_cond_t * cond)
{
  ...
  return(__ptw32_cond_unblock(cond, 1));
