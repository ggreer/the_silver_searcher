...
------------------------------------
- In private.c, added some code at the beginning of __ptw32_processInitialize
to detect the case of multiple calls to __ptw32_processInitialize.
...
In that case, the application must call __ptw32_rocessInitialize()
explicitely, to initialize pthread-win32.  It is safer in this circumstance
to handle the case where __ptw32_processInitialize() is called on
...
__ptw32_processInitialize (void)
{
	if (__ptw32_processInitialized) {
		 ...
		 * library. such programs must call __ptw32_processInitialize() explicitely,
