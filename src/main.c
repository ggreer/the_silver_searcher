...
int main(int argc, char **argv) {
    ...
    if (opts.literal) {
        ...
        if (bUsedDef) { // Some Unicode characters could not be converted to the CP_ACP code page.
          /* This means that the regular expression cannot match anything in CP_ACP input text.
             But the default character '?' generated by the conversion routine _could_ match something.
             So make sure that the matching will always quickly fail */
          free(opts.query2);
